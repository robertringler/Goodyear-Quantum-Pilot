...
class CuringResult:
    ...
    def is_acceptable(self) -> bool:
        ...
        return (
            np.mean(self.cure_state) > 0.9
            and self.reversion_index < 0.1
...
class CuringSimulation:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized CuringSimulation: {n_layers} layers, "
            f"thickness={tire_thickness} mm"
        )
        ...
        for step in range(n_steps):
            ...
            for i in range(1, self.n_layers - 1):
                T_new[i] = temperature[i] + (
                    self.alpha_thermal * dt * 60  # Convert to seconds
                    / (self.dx * 1e-3) ** 2
    ...
    def evaluate_cure_result(
        ...
        if result.reversion_index > 0.05:
            assessment["recommendations"].append(
                "Reduce cure time to prevent reversion"
            )
        
        if np.mean(result.cure_state) < 0.90:
            assessment["recommendations"].append(
                "Increase cure time or temperature"
            )

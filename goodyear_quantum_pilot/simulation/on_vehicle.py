...
class RollingDynamics:
    ...
    def __init__(
        ...
        logger.info(f"Initialized RollingDynamics: C_alpha={lateral_stiffness:.0f} N/rad")
...
class ThermalModel:
    ...
    def compute_heat_dissipation(
        ...
        A_tread = np.pi * self.geometry.overall_diameter * 1e-3 * self.geometry.tread_width * 1e-3
        A_sidewall = (
            2
            * np.pi
            * (
        ...
        Q_air_sidewall = (
            h_eff * A_sidewall * (thermal_state.sidewall_temp - load.ambient_temperature)
        )
...
class WearSimulator:
    ...
    def __init__(
        ...
        logger.info(f"Initialized WearSimulator: hardness={hardness} Shore A")
...
class OnVehicleSimulator:
    ...
    def simulate_drive_cycle(
        ...
        for condition in conditions:
            ...
            results["total_distance"] += condition.load_case.vehicle_speed * condition.duration

...
class QuantumBenchmark:
    ...
    def benchmark_vqe(
        ...
        for n_qubits in qubit_counts:
            for backend in backends:
                ...
                time_per_circuit = self._estimate_circuit_time(n_qubits, circuit_depth, backend)

                total_time = time_per_circuit * self.opt_iterations * self.shots
    ...
    def benchmark_qaoa(
        ...
        for size in problem_sizes:
            for p in p_values:
                for backend in backends:
                    time_per_circuit = self._estimate_circuit_time(size, p * 2, backend)

                    total_time = (
                        time_per_circuit
                        * 2
                        * p  # 2p parameters to optimize
                        * self.opt_iterations
                        * self.shots
                    )
                    ...
                    result = BenchmarkResult(
                        ...
                        backend=backend,
                        execution_time=total_time,
                        cost=self.HARDWARE_COSTS.get(backend, 0.0)
                        * self.shots
                        * self.opt_iterations,
                    )
                    results.append(result)
    ...
    def compute_quantum_advantage(
        ...
        cost_ratio = (
            classical_result.cost / quantum_result.cost if quantum_result.cost > 0 else float("inf")
        )
...
class ScalingAnalyzer:
    ...
    def compare_algorithms(
        ...
        comparison = {
            ...
            "scaling_exponents": {name: r.fitted_exponent for name, r in algorithm_results.items()},
            "r_squared": {name: r.r_squared for name, r in algorithm_results.items()},
            "complexity_classes": {name: r.scaling_class for name, r in algorithm_results.items()},
        }

...
class BackendComparison:
    ...
    def quantum_advantage_analysis(
        ...
        classical = [
            r
            for r in results
            if r.backend in [ComputeBackend.CPU, ComputeBackend.GPU_NVIDIA, ComputeBackend.GPU_AMD]
        ]
        quantum = [
            r
            for r in results
            if r.backend
            not in [ComputeBackend.CPU, ComputeBackend.GPU_NVIDIA, ComputeBackend.GPU_AMD]
        ]
        ...
        cost_ratio = (
            best_quantum.cost / best_classical.cost if best_classical.cost > 0 else float("inf")
        )
...
class AlgorithmComparison:
    ...
    def compare_convergence(
        ...
        return {
            ...
            "algorithms": comparison,
            "fastest_convergence": (
                min(comparison.items(), key=lambda x: x[1]["iterations"])[0] if comparison else None
            ),
        }
...
class MaterialComparison:
    ...
    def spider_chart_data(
        ...
        max_values = dict.fromkeys(properties, 0.0)

...
class QAOAResult:
    ...
    def get_blend_proportions(
        ...
        for i in range(n_materials):
            bits = self.optimal_bitstring[
                i * n_bits_per_material:(i + 1) * n_bits_per_material
            ]
            value = int(bits, 2)
...
class TireQAOA:
    ...
    def optimize(self) -> QAOAResult:
        ...
        logger.info(
            f"QAOA completed: optimal_value={best_cost:.4f}, "
    ...
    def get_blend_recommendation(
        ...
        """
        proportions = result.get_blend_proportions(
            discretization_levels=2 ** self.bits_per_material
        )

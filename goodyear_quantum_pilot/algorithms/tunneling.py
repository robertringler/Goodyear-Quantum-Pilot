...
class Barrier(ABC):
    ...
    def hessian(self, x: float, dx: float = 1e-6) -> float:
        ...
        return (self.potential(x + dx) - 2 * self.potential(x) + self.potential(x - dx)) / (dx**2)
...
class TunnelingSimulator:
    ...
    def compute_thermal_rate(
        ...
        transmissions = np.array([self._get_transmission(E) for E in energies])
    ...
    def compute_tunneling_rate(self) -> TunnelingResult:
        ...
        logger.info(f"Tunneling analysis: κ={kappa:.2f}, " f"lifetime={lifetime:.2e} s")

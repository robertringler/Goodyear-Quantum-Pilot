...
class StateVector(QuantumState):
    ...
    def __repr__(self) -> str:
        return (
            f"StateVector(num_qubits={self.num_qubits}, norm={np.linalg.norm(self.amplitudes):.4f})"
        )
...
class DensityMatrix(QuantumState):
    ...
    def thermal(cls, hamiltonian: np.ndarray, temperature: float) -> DensityMatrix:
        ...
        rho = np.linalg.matrix_power(np.diag(np.exp(-beta * np.linalg.eigvalsh(hamiltonian))), 1)

        ...
        while len(remaining) > 1:
            ...
            for i in range(len(remaining)):
                for j in range(i + 1, len(remaining)):
                    cost = tensors[remaining[i]].total_elements + tensors[remaining[j]].total_elements
                    if cost < best_cost:
                        best_cost = cost
    ...
    def _initialize_random(self) -> None:
        ...
        for i in range(self.num_sites):
            if i == 0:
                ...
                left_dim = min(self.bond_dimension, self.physical_dimension ** i)
                right_dim = min(self.bond_dimension, self.physical_dimension ** (self.num_sites - i - 1))
